...
import { addOutline, checkmark, checkmarkDoneOutline, imageOutline } from "ionicons/icons";
...
const UserPage: React.FC = () => {
	...
	socket.onmessage = (data:any)=>{
		...
		if(!message.status){
			if(message.filetype){
				cont[message.sender].lastMsg.push(message.filetype);
				cont[message.sender].sendStatus = 'file';
			}else{
				cont[message.sender].lastMsg.push(message.msg);
				cont[message.sender].sendStatus = 'rec';
			}
			cont[message.sender].time = message.sent;
			setContacts(cont);
			setNewMsg(message);
		}
	}
